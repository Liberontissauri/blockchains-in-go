...
func createHeader(data []byte, prevHash []byte, nonce uint64) *Header {
	return &Header{time.Now().Unix(), data, prevHash, nonce}
}
...
func (header *Header) getByteArray() []byte {
	return bytes.Join([][]byte {header.getByteTimestamp(), header.Data, header.PrevHash, header.getByteNonce()}, []byte{})
}
...
func (block *Block) isValid() bool{
	...
	if(big_int_hash.Cmp(&block.Target) == -1) {return true}
	return false
}
...
func CreateGenesisBlock(target_bits uint) *Block {
	...
	genesis_header := createHeader([]byte{0b00000000,0b00000000,}, []byte{}, 0)
	header_hash := crypto.SHA256.New()
